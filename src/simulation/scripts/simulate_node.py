...
def main():
    global last_pose_pub_time, last_obs_pub_time, robot_trajectory
    ...
    generate_landmarks(cylindrical_landmark_template_urdf, landmark_positions)
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            if not response.success:
                ...
                robot_trajectory.header.frame_id = robot_pose.header.frame_id = "odom"
        ...
        if (time.time() - last_obs_pub_time) >= (1/obs_pub_rate):
            last_obs_pub_time = time.time()
            observation_pub.publish(obs)
            
        if (time.time() - last_pose_pub_time) >= (1/pose_pub_rate):
            last_pose_pub_time = time.time()
